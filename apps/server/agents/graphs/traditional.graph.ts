...
import {
  StateGraph, // StateGraph：这是 LangGraph 中的核心组件，用于定义和管理工作流的状态、节点和边。每个节点执行一个具体的任务，而节点间的边决定了任务的执行顺序。
  START,
  END,
  MemorySaver,
  Annotation,
...
//  Annotation.Root():来声明整个图（Graph）中共享的状态字段，以及每个字段的更新方式（reducer）

const GraphState = Annotation.Root({
  // 基础字段 (LastValue 模式 - 默认)
  messages: Annotation<T_Graph["messages"]>({
    reducer: (x, y) => x.concat(y), //当多个节点同时写入 messages 时：如何合并: 旧值 + 新值
    default: () => [], // 默认值：空数组
  }),
